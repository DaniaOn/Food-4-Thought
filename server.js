...
sequelize.sync({ force: false }).then(() => {// Use the sync() method to connect to the database and create the tables based on the models
	app.listen(PORT, () =>// Use the listen() method to start the server and log a message to the console when the server has started 
		console.log(`\nServer running on port ${PORT}.`));// The \n character is used to add a new line to the console output 
});